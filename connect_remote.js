...
const handleLogin = (req,res) =>{
  con.query(`SELECT * FROM w_users WHERE user_name = '${req.query.user_name}'`, function (err, result, fields) {
    ...
    if(result.length > 0)
    {
	console.log(result[0].pass_word + ' = ' + req.query.pass_word);
      if(result[0].pass_word === req.query.pass_word)
      {
        var hash = crypto.createHash('md5').update(result[0].store_number + secret).digest('hex');
        var rslt = {user_name:result[0].user_name,store_number:result[0].store_number,hash:hash};
        res.send(rslt)
      }
      else{
